...
- **트랙 1 — Expo Web + Chrome 확장**: Claude 셀프 검증 / 디자인 매칭 / 빠른 반복.
  화면 1차 구현 직후 `npx expo start --web`으로 띄우고 Chrome 확장으로 풀페이지 스크린샷 1장만 찍어 디자인과 비교. **섹션별 캡처 금지** (토큰 소비). 로그인·모달 등은 직접 dismiss 후 작업 시작.
- **트랙 2 — Expo Go + 폰**: 사용자 최종 확인 / 네이티브 동작 검증. 화면이 디자인과 일치한 뒤 사용자가 폰에서 SafeArea·키보드·제스처를 실기 확인.

### 일반 규칙

- **UI 작업 후 트랙 1로 1차 셀프 확인** → 디자인과 일치하면 "완료(웹 검증)" 보고. 사용자에게 폰 검증을 요청.
- **트랙 1로 검증 안 되는 항목** (반드시 폰에서 확인 — 트랙 2 필수):
  - SafeArea (`useSafeAreaInsets`가 web에서 항상 0 반환)
  - 키보드 동작 (`KeyboardAvoidingView` 정상 작동 안 함)
  - 네이티브 제스처 (스와이프 백, 풀투리프레시)
  - SecureStore (web 미지원, 콘솔 경고 무시)
  - Reanimated 일부 동작
...
| ✅ `SysBar` deprecated                    | 작업 17.5에서 `src/components/ui/SysBar.tsx` 삭제 + index re-export 제거                   |
